...
<Anchor id="accepted_if">accepted_if:anotherfield,value</Anchor>
...
<Anchor id="declined_if">declined_if:another_value,value</Anchor>
...
Strings, numerics, arrays, and files are evaluated using the same conventions as the <a href="#size">size</a> rule.
...
so a number in a <code>string</code> is also considered as numeric. If you need to validate the the input is of type <code>number</code>, 
...
This method accepts a booelan or a closure. When passed a closure, the closure should return <code>true</code> or <code>false</code>
...
<Anchor id="strats_with">starts_with:<em>foo,bar...</em></Anchor>
...
The field udner validation must be a valid URL.
